...
ENVELOPE_CUTOFF = 2.5  # Low-pass filter cutoff for envelope extraction (in Hz)
...
try:
    while True:
        ...
        if time.time() - start_time >= 10:
            mod = int(len(bin)) % (sampling_time*SAMPLING_RATE)
            if mod == 0 and len(bin) != 0:
                #print("hawk tuah")
                
                bin = np.array(bin)
                filtered_bin = apply_bandpass_filter(bin, LOW_CUTOFF, HIGH_CUTOFF, SAMPLING_RATE)
                filtered_bin = np.clip(filtered_bin, -20, 20)
                envelope = calculate_envelope(filtered_bin, SAMPLING_RATE, ENVELOPE_CUTOFF)
                envelope_peaks, _ = find_peaks(envelope)
                if len(envelope_peaks) != 0:
                    print(f"Contraction Detected at time: {time.time() - start_time}")
                bin = []
            else:
                bin.append(value)
